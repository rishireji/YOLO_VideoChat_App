...
export const LandingPage: React.FC<LandingPageProps> = ({ onStart }) => {
  ...
  return (
    <div className="relative w-full bg-[#0a0a0c]">
      {/* 
        Hero Section with Lamp Container providing the primary studio lighting effect.
      */}
      <section className="relative min-h-screen">
        <LampContainer className="min-h-screen">
          ...
          >
            <div className="inline-block px-4 py-1.5 mb-6 rounded-full border border-amber-500/20 bg-amber-500/5 text-amber-500/60 text-[10px] font-black tracking-[0.4em] uppercase">
            ...
            <div className="w-full max-w-md mx-auto bg-black/40 backdrop-blur-3xl border border-white/10 p-8 rounded-[40px] shadow-[0_30px_100px_rgba(0,0,0,0.8)] space-y-6 relative z-[60]">
              <div className="space-y-2 text-left">
                ...
                <div className="relative group">
                  <select 
                    ...
                    className="w-full bg-zinc-900/40 border border-zinc-800 text-white rounded-2xl px-5 py-4 focus:ring-2 focus:ring-amber-600 outline-none transition-all appearance-none cursor-pointer pr-10 hover:border-zinc-600 font-bold"
                  >
              ...
              <div className="flex items-start gap-4 text-left group cursor-pointer p-2 rounded-2xl hover:bg-white/5 transition-colors" onClick={() => setAcceptedTerms(!acceptedTerms)}>
                <div className={`mt-1 flex-shrink-0 w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all ${acceptedTerms ? 'bg-amber-600 border-amber-600 shadow-[0_0_15px_rgba(245,158,11,0.2)]' : 'border-zinc-700 bg-zinc-900'}`}>
                ...
                <label className="text-xs text-zinc-500 select-none cursor-pointer leading-relaxed group-hover:text-zinc-300 transition-colors">
                  I agree to the <span className="text-amber-500 font-black">AI Safety Compliance</span> and confirm I am 18+.
                </label>
              </div>
              ...
              <button
                ...
                className={`w-full py-5 rounded-2xl font-black text-lg transition-all transform active:scale-95 flex items-center justify-center gap-3 ${
                  acceptedTerms
                    ? 'bg-amber-600 text-white hover:bg-amber-500 shadow-[0_0_30px_rgba(245,158,11,0.2)]' 
      ...
      {/* Cyber Background providing subtle depth for content sections */}
      <div className="relative">
        ...
        <section className="relative z-10 max-w-6xl mx-auto px-6 py-32 grid md:grid-cols-3 gap-8">
          {[
            ...
            <div key={idx} className="bg-black/20 backdrop-blur-3xl border border-white/5 p-10 rounded-[48px] hover:border-amber-500/10 transition-all hover:-translate-y-2 group shadow-2xl relative z-10">
              <div className={`w-14 h-14 bg-amber-600/5 rounded-2xl flex items-center justify-center mb-8 text-amber-500/60 group-hover:scale-110 transition-transform shadow-[0_0_20px_rgba(245,158,11,0.02)] border border-amber-500/5`}>
        ...
        <section id="faq-section" className="relative z-10 max-w-4xl mx-auto px-6 pb-32 pt-10">
          <div className="text-center mb-20">
            ...
            <div className="w-24 h-1 bg-amber-600/20 mx-auto rounded-full shadow-[0_0_20px_rgba(245,158,11,0.1)]"></div>
          </div>

          <div className="bg-black/20 border border-white/5 rounded-[56px] p-8 md:p-16 backdrop-blur-3xl shadow-inner relative z-10">
            {faqs.map((faq, index) => (
      ...
      {/* Footer Branding */}
      <footer className="py-20 border-t border-white/5 text-center relative z-10 bg-black/40">
