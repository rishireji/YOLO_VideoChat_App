...
import { RevealRule, UserProfile, UserFile } from '../types';
...
export const ProfilePage: React.FC<ProfilePageProps> = ({ isOpen, onClose }) => {
  const { user, profile, files, updateProfile, logout, deleteAccount, uploadVaultFile, deleteVaultFile } = useAuth();
  const { session } = useSession();
  const [activeTab, setActiveTab] = useState<'identity' | 'files' | 'wallet' | 'privacy'>('identity');
  ...
  const handleVaultUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      try {
        ...
        console.error("[YOLO Vault] Upload failed:", err);
      } finally {
        setUploading(false);
      }
    }
  ...
  const removePhoto = (field: 'Display_Pic1' | 'Display_Pic2' | 'Display_Pic3') => {
        ...
        <section className="bg-zinc-950/40 backdrop-blur-3xl border border-white/10 rounded-[56px] p-8 md:p-14 shadow-[0_40px_100px_rgba(0,0,0,0.8)] relative overflow-hidden min-h-[600px]">
            ...
            <div className="space-y-14 animate-in fade-in slide-in-from-bottom-6 duration-700">
               ...
               <div className="space-y-6">
                 <div className="flex items-center justify-between">
                    <div>
                      <h4 className="text-xl font-outfit font-bold text-zinc-100 mb-1 uppercase tracking-tight">Vault Display Frames</h4>
                      <p className="text-zinc-600 text-xs font-medium uppercase tracking-wide">Secondary pics synced to '/Users/{profile.uid}'</p>
                    </div>
                 </div>
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-700">
               <div className="flex items-center justify-between">
                  ...
                  <button 
                    ...
                    disabled={uploading}
                    className="flex items-center gap-3 px-8 py-4 bg-white text-black font-black uppercase tracking-widest rounded-2xl hover:bg-zinc-200 transition-all active:scale-95 disabled:opacity-50"
                  >
                    {uploading ? (
                      <div className="w-5 h-5 border-2 border-zinc-600 border-t-zinc-950 rounded-full animate-spin"></div>
                    ) : (
                        ...
                        <div className="flex flex-col md:flex-row gap-8">
                           <div className="flex-1 space-y-4">
                              ...
                              <div className="p-5 bg-zinc-950/80 rounded-[28px] border border-white/5 relative">
                                 <div className="absolute top-4 right-4">
                                    <span className="text-[8px] font-black uppercase text-red-500/50 tracking-widest">Gemini AI Summary</span>
                                 </div>
                           ...
                           <div className="flex md:flex-col gap-2 justify-center">
                              <a 
                                ...
                                className="p-4 bg-zinc-950 border border-white/5 text-zinc-400 hover:text-white rounded-2xl transition-all"
                                title="Download Protocol"
                              >
                                 <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                              </a>
                              <button 
                                onClick={() => deleteVaultFile(file.id, file.storagePath)} 
                                className="p-4 bg-red-600/10 border border-red-500/10 text-red-500 hover:bg-red-600 hover:text-white rounded-2xl transition-all"
          ...
          {activeTab === 'wallet' && (
            <div className="space-y-12 animate-in fade-in slide-in-from-bottom-6 duration-700">
               <h3 className="text-4xl font-outfit font-black text-white italic">ENERGY VAULT</h3>
          ...
          {activeTab === 'privacy' && (
            <div className="space-y-12 animate-in fade-in slide-in-from-bottom-6 duration-700">
               <h3 className="text-4xl font-outfit font-black text-white italic">SIGNAL STEALTH</h3>
               <div className="space-y-4">
                 {[
                   ...
                   <div key={setting.id} className="flex items-center justify-between p-10 bg-zinc-950/50 border border-white/5 rounded-[48px]">
                      ...
                      <button 
                        ...
                        className={`w-16 h-9 rounded-full p-1.5 transition-all flex items-center ${setting.value ? 'bg-red-600 justify-end' : 'bg-zinc-900 justify-start'}`}
                      >
