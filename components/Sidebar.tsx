...
import { ChatMessage, Region, REGION_LABELS, DAILY_ALLOWANCE } from '../types';
...
export const Sidebar: React.FC<SidebarProps> = ({ messages, onSendMessage, onToggleTranslation, isConnected, region }) => {
  ...
  const totalCoins = (session?.coins || 0) + (session?.purchasedCoins || 0);
  ...
  return (
    <div className="flex flex-col w-full lg:w-[400px] bg-zinc-950/50 lg:border-l border-t lg:border-t-0 border-zinc-800/50 h-[50vh] lg:h-full backdrop-blur-3xl shadow-2xl relative z-30 transition-all duration-700">
      <div className="p-4 lg:p-6 border-b border-zinc-800/50 flex flex-col gap-4 bg-zinc-950/80 sticky top-0 z-10">
        ...
        <div className="space-y-1.5 px-0.5">
          <div className="flex items-center justify-between text-[8px] font-black uppercase tracking-[0.3em] text-zinc-600">
            ...
            <span className={totalCoins < 20 ? 'text-red-500 animate-pulse' : 'text-zinc-400'}>{totalCoins} YC</span>
          </div>
